...
HRESULT npcTeacher::init(float x, float y, STATE state, DIRECTION direction)
{
	//이미지 및 애니메이션 담기
	_Idle = IMAGEMANAGER->findImage("NpcTeacherIdle");
	_React = IMAGEMANAGER->findImage("NpcTeacherReact");
...
void npcTeacher::render()
{
}

void npcTeacher::render(POINT camera)
{
	npc::render(camera);
}
